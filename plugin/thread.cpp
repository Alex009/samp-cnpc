...
void SAMP_ThreadComplete()
{
	...
	for(playerid = 0;playerid < MaxPlayers;playerid++)
	{
		...
		case PLAYER_STATE_ONFOOT:
			{
				...
				// определяем данные стрельбы в зависимости от оружия
				...
				default:
					{
						// получаем координаты камеры и вектор
						...
						// в зависимости от оружия смещаем вектор
						switch(weaponid)
						{
						case WEAPON_SNIPER:
		...
		// есть ли необходимость просчёта урона для NPC
